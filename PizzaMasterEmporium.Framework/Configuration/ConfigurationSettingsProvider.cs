        ...
        internal void SetProperty(PropertyInfo propertyInfo, object target, object value)
        {
            switch (propertyInfo.PropertyType.Name)
            {
                ...
                case "Boolean":
                    ...
                    break;
                default:
                    _logger.LogErrorMessage(this.GetType(), null, string.Format("Can't convert {0} to {1} for property {2}.{3}", value, propertyInfo.PropertyType.Name, target.GetType().FullName, propertyInfo.Name));
                    return;
            }
        }
    }
